...
5. **Close onboarding with the tour, not a lecture.** Recap what now exists (profile, brag doc, positioning, channel plan, tracker). Then hand them the manual and the loop in three sentences: point at `docs/using-jobsmith.md`, explain that they move rows in the tracker and you catch up at session start, and give them two or three example things to say next, matched to their situation ("Run a sweep" · "Build kits for anything you approve" · "Let's finish your brag doc"). Thank them and mark onboarding complete in `my/state.json`. If they arrived with a job description in hand, offer a tailored look using the brag doc you built together.
